...
| BUG-CULL-01 | Modellvalet nådde aldrig gallringen (etikett vs typnyckel) | ✅ **LÖST 17/7** (`1ff5a07` main, pushad): `_modell_typ` normaliserar UI-etikett→typnyckel (täcker gamla sparade jobb), selecten skickar typnycklar, vald-men-oladdbar modell loggas högt ("⚠ kunde inte laddas"). **Skarpverifierad i pipx-miljön mot live-DB:** Din smak + Arkiv laddar riktiga paket, arkiv bär sportmodell för FOTBOLL → tränade modellen används nu på riktigt. Stig: DPT2-omstart. Kvar (medvetet): Hybrid otränad (ger nu synlig ⚠-rad) + pkl_path pekar på gamla `~/.config/dpt/modeller/` (funkar, men skört — flytt vid tillfälle) |
