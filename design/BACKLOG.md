...
|----|-----|-----|-----------|
| F20-5 | Deltillfälle-arket: slå ihop **"Skapa SoMe" + "Öppna passet"** till EN vy m stjärnmärkta deltagare överst; **scoring per passtyp** (kval → bästa tid/placering; final → vinnare finns bara i finaler) | förbättring/fråga | iOS `GrenArk`/`FaltflodeView` (J-2/M-10). **SCORINGEN OBLOCKERAD 20/7:** M-6:s datamodell-keystone (`a8faee8`, v45) gav `disciplin_deltagare` resultat/placering/medalj + `disciplin_deltagare`-listan rankas nu på placering. **KVAR för F20-5:** tråda resultat/placering/medalj i match/tävlings-paketet → iOS `FriDeltagare` avkodar dem → scoring i `GrenArk` (final→medalj överst, kval→placering/resultat). Vy-mergen (Skapa SoMe + Öppna passet) = visuell/maskin |
