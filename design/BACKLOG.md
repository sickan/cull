...
| F18-3 | "Importera spelschema"-knappen död (DPT2) | ✅ **HANDLER + LOGIK KLAR 20/7** (dpt `b1d0aca`): `store.importera_spelschema(fixtures, sport)` + app-metod — loopar `spara_match` (normaliserar liga→tävling, lagnamn→lag; inget dubbleras), idempotent på liga+hemma+borta+datum. 3 tester. **Handbollsligan Herr 2026/27** (15 matcher, HK Malmö+LUGI) sparad i `dpt/design/import/handbollsligan-herr-2026-2027.json`, redo att importera. **KVAR (visuellt):** Svelte-knapp + filväljare/inklistring i Matcher-panelen |
