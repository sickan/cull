...
| BUG-CULL-01 | **Modellvalet når ALDRIG gallringen** → varje cull faller till den handsatta fotbollsformeln oavsett vald modell | **BEKRÄFTAD 17/7 (kodläst + live-DB).** Rot: UI:t skickar VISNINGSNAMN (`"Din smak"`/`"Arkiv (facit)"`/`"Hybrid"`) som `cull_jobb.modell`, men `_modell_paket` matchar på TYPNYCKEL (`din_smak`/`arkiv`/`hybrid`) → aldrig träff → `None` → "Ingen tränad modell — handsatt poängformel". Modellerna FINNS (`arkiv.pkl`, `din_smak.pkl` i DB+disk). Förklarar Stigs "Träna har inte fångat gallringen" + identiska resultat mellan Arkiv/Din smak. **Liten fix (label→key-mappning), hög effekt.** OBS: `_modell_paket` saknar även Hybrid-logik; pkl_path pekar på gamla `~/.config/dpt/modeller/` (skört) |
