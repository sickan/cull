...
|----|-----|-----|-----------|
| F20-5 | Deltillfälle-arket: slå ihop **"Skapa SoMe" + "Öppna passet"** till EN vy m stjärnmärkta deltagare överst; **scoring per passtyp** (kval → bästa tid/placering; final → vinnare finns bara i finaler) | förbättring/fråga | iOS `GrenArk`/`FaltflodeView` (J-2/M-10). **⛔ SCORINGEN BLOCKERAD (upptäckt 20/7):** `FriDeltagare` bär bara namn/klubb/gren/handle — **ingen resultat/placering/medalj per deltagare**, så kval-tid/final-vinnare går inte att ranka. Det är **M-6:s datakrav** (P3) → F20-5-scoringen beror på M-6. Passtyp (kval/final) går att läsa ur namnet, men det räcker inte ensamt. **Bara vy-mergen är P1 (visuell/maskin)**; scoringen väntar på M-6 |
