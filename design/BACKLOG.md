...
| ID | Vad | Anteckning |
|----|-----|-----------|
| V2-19 | **BUGG AKUT:** Malmö FF–Bröndby (Eleda, **lör 18/7 = imorgon**) syns i listvyn men INTE i kalendervyn; 18:e saknar jobbprick + väder. Ledtråd: posten visar ingen datumtext i listan → saknat/felformaterat datumfält; kalender + lista ska läsa SAMMA fält; poster utan giltigt datum flaggas synligt, försvinner aldrig tyst | v2 §19 · öppen fråga: lades matchen in på annan väg? |
| V2-16 | **BUGG:** "Dela till Instagram Stories" felar ("går inte att skicka filen") trots ok bild — felet i delningsvägen. Fix: `instagram-stories://share?source_application={bundle-id}` + bilddata på UIPasteboard som `com.instagram.sharedSticker.backgroundImage` (PNG/JPG-DATA, ej fil-URL) + `instagram-stories` i LSApplicationQueriesSchemes. **Interim först: "Spara till Bilder"-knapp** bredvid delningsknappen | v2 §16 · slår direkt mot story-funktionens värde; absorberar gamla IG-schema-raden |
| V2-05 | Förhandsgranskning vid SoMe-generering hamnar ovanför synfältet — man ser inte att den skapats. Väg: autoscroll när klar / rendera där man står / tydlig "Visa förhandsgranskning"-indikator | v2 §5 · ta ihop med V2-16-flödet |
