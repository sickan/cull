...
**BORTTAGNA 20/7:** B-003 (→ Dala/K-3) · SPIKE-03/04/05/06 (obsoleta) · SPIKE-07 (→ FEAT-10).
...
| M-6 | **Utövarsidan omtänkt** (`DPT v5 - Utövare`) — profil m klass-färgkant + **inline @-fält** ("sätts en gång — fältflödet taggar automatiskt sen", ✓ *bär till fältet*), fyra nyckeltal (starter · tävlingar · bilder · persrekord i accent), **Kommande starter** (härledda pass), **Historik** (härledd tidslinje, **medalj = accent-prick + accent-resultat**), **Bilder & jobb** (3-kolumnsgrid, klick → jobbet), "Nås ifrån"-pills | 🔻 P3, men **DATAMODELL-KEYSTONE KLAR 20/7** (dpt `a8faee8`, **schema v45**): `disciplin_deltagare` bär nu **resultat/placering/medalj** (`satt_disciplin_resultat`, listan rankas på placering); `utovare_historik` (starter m resultat + tävlingskontext) + `utovare_persrekord` (härlett). `tavling.ort/datum` fanns redan. 5 tester. **Detta låste även upp F20-5.** Sidan FINNS (`4398b0b`) men byggdes före D12 → omtag ihop m M-1/M-2. **KVAR:** sätt-resultat-UI (Svelte, maskin) · **bildtaggning person→bild→jobb** (egen större slice) för "Bilder & jobb" · utövarside-UI (iOS/Svelte, visuellt) |
...
| W-11 | **Sol- och väderklicket når inte rätt app** — `dpt://sol` landar i App Store i stället för Sun Surveyors rätta plats/datum, `dpt://vader` öppnar yr.no i Safari i stället för Yr-appen | ⚠️ **ÖPPET, kräver Stig.** Roten är inte vår kod: **varken Yr eller Sun Surveyor har ett dokumenterat URL-schema.** Verifierat 19/7 — `yr.no/.well-known/apple-app-site-association` ger **404** (⇒ inga universal links heller, en https-länk kan aldrig öppna appen), och sunsurveyor.com dokumenterar varken scheme eller API. Enda officiellt dokumenterade är `comgooglemaps://` (Google), som arenaklicket använder och som fungerar. Widgeten länkar därför till `dpt://vader|sol?lat=&lon=` och **appen är router** (`Sources/Externa.swift`): provar gissade kandidater med `canOpenURL`, faller tillbaka på yr.no med lat/lon i sökvägen (verifierad 200) resp. App Store `id525176875`. Ingen knapp blir död — men ingen träffar rätt heller. Kandidaterna KRÄVER `LSApplicationQueriesSchemes` i project.yml, annars svarar `canOpenURL` alltid false. Priset för router-lösningen: **DPT blinkar förbi en sekund** före målappen (Stigs val 19/7). **Åtgärd: mejla adam@sunsurveyor.com** (enmansutvecklare) och fråga efter schemat — då är det fem minuters arbete att koppla in. Yr har ingen känd väg alls; där är Safari-fallbacken troligen slutstationen |
...
| F18-3 | "Importera spelschema"-knappen död (DPT2) | ✅ **HANDLER + LOGIK KLAR 20/7** (dpt `b1d0aca`/`bd3284f`): `store.importera_spelschema(fixtures, sport)` + app-metod — loopar `spara_match` (normaliserar liga→tävling, lagnamn→lag; inget dubbleras), idempotent på liga+hemma+borta+datum. Sport per fixture → arg → **liganamnet** (engelska mappas). 6 tester. **Fyra scheman 2026/27** sparade i `dpt/design/import/`: handbollsligan-herr (15) · handbollsligan-dam (18) · elitserien-herr (10) · elitserien-damer (9). **KVAR (visuellt):** Svelte-knapp + filväljare i Matcher-panelen |
| KROCK | **Krock-hantering vid inläsning** (Stig 20/7) — bevakade matcher samma dag = du kan inte vara på två ställen | ✅ **LOGIK KLAR 20/7** (dpt `7942445`): **bevakningsprio** HK Malmö > Lunds VK Dam > H65 > Lunds VK herr > OV (Lunds VK skiljs på LIGA, inte lagnamn). `store.match_prio` (lag mot hemma/borta + ev. liga-krav) + `hitta_krockar` — grupperar per **ARENA (=hemmalaget)**: **samma hall = dubbelmatch** (Lunds VK dam+herr efter varandra), krock bara mellan OLIKA arenor samma dag; obevakade lag ignoreras. Returstruktur `{datum, alternativ:[{arena,prio,matcher[]}…], forslag}`. `importera_spelschema`+`bevakningskrockar()` returnerar dem. 5 tester. **Reellt: 8 krock-datum** i höstscheman (7/11 Lund buntar BÅDA Lunds VK över H65; 12/19 två Lunds VK-hemmamatcher = dubbelmatch, EJ krock; 9/10 HK Malmö över OV). Släkt m [[project-privata-kalendrar]]-krockkoll + F20-9. **FRAMTIDA IMPORT-BETEENDE (Stig 20/7):** import-knappen ska INTE auto-importera-och-lösa. Den ska **visa krockarna för BESLUT** — per krock fyra val: **den ena · den andra · BÅDA · importera inte**. Prio-listan (`match_prio`) ger bara FÖRSLAGET (default-markering), Stig beslutar — samma godkänn-steg som Generera/Trupp-OCR. Icke-krockande matcher importeras normalt; krockande hålls tills beslut (skiljer sig från nuvarande auto-import). **KVAR (visuellt/maskin):** besluts-UI i Matcher-panelen + redigerbar prio-lista |
...
|----|-----|-----|-----------|
| F20-5 | Deltillfälle-arket: slå ihop **"Skapa SoMe" + "Öppna passet"** till EN vy m stjärnmärkta deltagare överst; **scoring per passtyp** (kval → bästa tid/placering; final → vinnare finns bara i finaler) | förbättring/fråga | iOS `GrenArk`/`FaltflodeView` (J-2/M-10). **SCORING-DATAT KLART HELA VÄGEN 20/7:** M-6-keystone (`a8faee8`, v45) + paket-trådning (dpt `08289e3`) + iOS (`da6ba6f`): `FriDeltagare` bär resultat/placering/medalj (rankat på placering ur DPT2), och `Programpunkt.passtyp`/`.medaljorer`/`.topp(n)` = scoring-primitiven (final→medaljörer, kval→går-vidare, semi=kval). 6 tester. **KVAR (bara visuellt/maskin):** koppla in primitiven i `GrenArk` + vy-mergen (Skapa SoMe + Öppna passet till EN vy) + sätt-resultat-UI i DPT2 (Svelte). Logiken + datat är gjort |
...
| F20-7 | Väder vid arenan: EN datapunkt 4 dagar innan. Så nära inpå bör fler punkter/dag finnas; annars åtminstone rätt DAGAR (24–26/7) representerade | bug/förbättring | ✅ **LOGIK KLAR 20/7** (ios `00b4dfb`, 219 gröna): roten var att `vadertider` gav 4 punkter inom 5 h av första dagen → långt fram (MET 6 h-steg) kollapsade de till EN (F-8/J-8). Nu: flerdagars heldag → **middag per dag** (24 h isär → skilda MET-luckor → varje tävlingsdag syns), endags behåller −1h/START/+2h/+4h. **KVAR:** widgetens event-väder (`SnapshotSkrivare`) samma per-dag-behandling · visuell verifiering på device |
